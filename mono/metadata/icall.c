...
ves_icall_System_Environment_GetEnvironmentVariable_native (const gchar *utf8_name, MonoError *error)
{
	...
	// The following 'if' statement and contents is an ANSYS patch to MONO.
 	// It is required because the QRSH_COMMAND, generated by the UGE Cluster
 	// library, may contain illegal characters, which this patch fixes.
 	// Without this change, certain operations on UGE clusters may fail.
 	if (strcmp(utf8_name, "QRSH_COMMAND") == 0 && strlen(value) > 0) {
 		const gchar invalidChar = (gchar)0xFF;
 		gchar *nextInvalidChar = strchr(value, invalidChar);
 		while(nextInvalidChar != NULL)
 		{
 			*nextInvalidChar = ' ';
 			nextInvalidChar = strchr(nextInvalidChar+1, invalidChar);
 		}
 	}
	...
	// The following 'if' statement and contents is an ANSYS patch to MONO.
 	// It makes it easier to debug issues caused by illegal environment variable value
 	if (!is_ok (error)) {
 		g_warning ("MONO ves_icall_System_Environment_GetEnvironmentVariable_native - Environment variable '%s' has value that could not be converted to UTF-8", utf8_name);
 	}
...
{
	return (gpointer)mono_lookup_internal_call_full (method, FALSE, NULL, NULL);
}
